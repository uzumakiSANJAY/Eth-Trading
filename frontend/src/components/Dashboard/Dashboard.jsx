...
const Dashboard = () => {
  ...
  const { currentPrice, isConnected, autoSignal, autoMtf, autoReview, autoPaper, autoVolatility, autoLastUpdated } = useWebSocketStore();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          {autoVolatility && (() => {
            const regime = autoVolatility.regime;
            const squeeze = autoVolatility.bbSqueeze;
            const colors = {
              LOW:     'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 border-green-300 dark:border-green-700',
              NORMAL:  'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-700',
              HIGH:    'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-700',
              EXTREME: 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 border-red-300 dark:border-red-700',
            };
            const icons = { LOW: '🟢', NORMAL: '🔵', HIGH: '🟠', EXTREME: '🔴' };
            return (
              <div className={`px-3 py-2 rounded-lg border ${colors[regime] || colors.NORMAL} text-center`}>
                <p className="text-xs font-semibold uppercase tracking-wide opacity-70">Volatility</p>
                <p className="text-sm font-bold">{icons[regime] || '●'} {regime}</p>
                {squeeze?.isSqueezing && (
                  <p className="text-xs font-semibold mt-0.5 animate-pulse">
                    ⚡ BB {squeeze.intensity} squeeze
                  </p>
                )}
              </div>
            );
